...
**Replace MLP feed-forward layers with Kolmogorov–Arnold Networks (KANs) in Transformers for character-level language modeling.**

This repository contains code to train and evaluate Transformer language models where the standard MLP feed-forward layers are replaced with spline-based KAN layers. The implementation is based on the architecture presented in the paper *"KAN-Transformers: Efficient Language Modeling with Kolmogorov–Arnold Networks"*.
...
where each KAN linear transformation \(\Phi\) implements learnable activation functions using B-splines:
